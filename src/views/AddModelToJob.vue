<template>
  <div>
    ...
    <input
      v-model="ModelJobForm.ModelIdNumber"
      class="input"
      type="number"
    ...
    /><br />
    <input
      v-model="ModelJobForm.JobIdNumber"
      class="input"
      type="number"
      placeholder="Job Id"
      min="1"
    ...
    <button v-on:click="AddModelJob" class="create">Add Model to job</button>
    <br />
    <button v-on:click="RemoveModelJob" class="create">
  ...
  methods: {
    async load() {
      ...
      try {
        ...
        alert(
          "Could not load data. Maybe you don't have permission for this. "
        );
      }
    },
    AddModelJob() {
      var url =
        ...
        this.ModelJobForm.JobIdNumber +
        "/model/" +
        this.ModelJobForm.ModelIdNumber;
      try {
        fetch(url, {
          method: "POST",
    ...
    RemoveModelJob() {
      ...
      try {
        fetch(url, {
          ...
          this.response = responseJson;
          if (this.response.status >= 200 && this.response.status < 300)
            alert("Successfully removed model from job");
